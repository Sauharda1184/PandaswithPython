...
def select_direction():
    ...
    print("Press 1 for South Bound (SB)")
    print("Press 2 for East Bound (EB)")
    print("Press 3 for North Bound (NB)")
    print("Press 4 for West Bound (SB)")
...
def collect_traffic_data():
    ...
    file_name = "intersection_traffic_testing.csv"
    ...
    def submit_data():
        # Get the time from dropdown selection
    ...
    # Generate time options in 15-minute intervals from 5:00 AM to 8:00 PM
    time_options = []
    for hour in range(5, 21):  # 5 AM to 8 PM (20:00)
        for minute in [0, 15, 30, 45]:
            # Format time as HH:MM AM/PM
            am_pm = "AM" if hour < 12 else "PM"
            display_hour = hour if hour <= 12 else hour - 12
            display_hour = 12 if display_hour == 0 else display_hour  # Handle 12 PM
    ...
    time_var.set(time_options[0])  # Default value
