...
class App:
    ...
    def show_mapping_dialog(self):
        ...
        mapping_dialog.title("Configure Zone Name Mappings")
        ...
        instructions = tk.Label(mapping_dialog, font = ft, justify = "left",
                              text = "Map camera positions to directions:\nFormat: Zone prefix = Direction (EB, WB, NB, SB)")
        instructions.place(x = 20, y = 10, width = 350, height = 40)
        
        # Camera 1 mapping
        cam1_label = tk.Label(mapping_dialog, font = ft, text = "Camera 1:")
        cam1_label.place(x = 20, y = 60, width = 80, height = 25)
        
        cam1_entry = tk.Entry(mapping_dialog, font = ft)
        cam1_entry.place(x = 110, y = 60, width = 60, height = 25)
        cam1_entry.insert(0, "EB")
        
        # Camera 2 mapping
        cam2_label = tk.Label(mapping_dialog, font = ft, text = "Camera 2:")
        cam2_label.place(x = 20, y = 95, width = 80, height = 25)
        
        cam2_entry = tk.Entry(mapping_dialog, font = ft)
        cam2_entry.place(x = 110, y = 95, width = 60, height = 25)
        cam2_entry.insert(0, "WB")
        
        # Camera 3 mapping
        cam3_label = tk.Label(mapping_dialog, font = ft, text = "Camera 3:")
        cam3_label.place(x = 20, y = 130, width = 80, height = 25)
        
        cam3_entry = tk.Entry(mapping_dialog, font = ft)
        cam3_entry.place(x = 110, y = 130, width = 60, height = 25)
        cam3_entry.insert(0, "NB")
        
        # Camera 4 mapping
        cam4_label = tk.Label(mapping_dialog, font = ft, text = "Camera 4:")
        cam4_label.place(x = 20, y = 165, width = 80, height = 25)
        
        cam4_entry = tk.Entry(mapping_dialog, font = ft)
        cam4_entry.place(x = 110, y = 165, width = 60, height = 25)
        cam4_entry.insert(0, "SB")
        
        # Custom prefix mappings
        prefix_label = tk.Label(mapping_dialog, font = ft, text = "Custom Prefixes:")
        prefix_label.place(x = 180, y = 60, width = 120, height = 25)
        
        # TRT mapping
        trt_label = tk.Label(mapping_dialog, font = ft, text = "TRT:")
        trt_label.place(x = 190, y = 95, width = 40, height = 25)
        
        trt_entry = tk.Entry(mapping_dialog, font = ft)
        trt_entry.place(x = 240, y = 95, width = 60, height = 25)
        trt_entry.insert(0, "EB")
        
        # ADV mapping
        adv_label = tk.Label(mapping_dialog, font = ft, text = "ADV:")
        adv_label.place(x = 190, y = 130, width = 40, height = 25)
        
        adv_entry = tk.Entry(mapping_dialog, font = ft)
        adv_entry.place(x = 240, y = 130, width = 60, height = 25)
        adv_entry.insert(0, "WB")
        
        # TI mapping
        ti_label = tk.Label(mapping_dialog, font = ft, text = "TI:")
        ti_label.place(x = 190, y = 165, width = 40, height = 25)
        
        ti_entry = tk.Entry(mapping_dialog, font = ft)
        ti_entry.place(x = 240, y = 165, width = 60, height = 25)
        ti_entry.insert(0, "NB")
        ...
        def save_mappings():
            ...
            status_text.set(f"Custom mappings saved: TRT={trt_entry.get()}, ADV={adv_entry.get()}, TI={ti_entry.get()}")
            status_message["text"] = status_text.get()
            mapping_dialog.destroy()
        
        # Save button
        save_button = tk.Button(mapping_dialog, font = ft, text = "Save", command = save_mappings)
        save_button.place(x = 170, y = 220, width = 60, height = 30)
        
        # Cancel button
        cancel_button = tk.Button(mapping_dialog, font = ft, text = "Cancel", 
                                 command = mapping_dialog.destroy)
        cancel_button.place(x = 240, y = 220, width = 60, height = 30)
    ...
    def Submit_button_command(self):
        ...
        def map_zone_to_direction(zone_name, camera_number, direction_list):
            ...
            # Get the prefix from direction_list based on camera number
            prefix = direction_list[camera_number-1] if camera_number-1 < len(direction_list) else "NA"
            
            # Default mapping dictionary
            default_mapping = {
                # Map each camera number to its corresponding direction
                1: "EB",
                2: "WB", 
                3: "NB",
                4: "SB",
                # Map common non-standard zone prefixes to directions
                "TR": "EB",
                "AD": "WB",
                "TI": "NB",
                "TRT": "EB",
            ...
            # Use custom mappings if they exist, otherwise fall back to defaults
            direction_mapping = custom_direction_mapping if 'custom_direction_mapping' in globals() else default_mapping
            
            # First check if the zone name prefix (first 2-3 chars) is in our mapping
            for prefix_len in range(3, 1, -1):
                if len(zone_name) >= prefix_len:
                    zone_prefix = zone_name[:prefix_len]
                    if zone_prefix in direction_mapping:
                        return direction_mapping[zone_prefix] + zone_name
            
            # Try to map by the prefix from direction_list
            if prefix in direction_mapping:
                return direction_mapping[prefix] + zone_name
        
            # If camera number is 1-4, map to standard directions
            if camera_number in direction_mapping:
                return direction_mapping[camera_number] + zone_name
        
            # Default to original zone name if no mapping found
            return zone_name
